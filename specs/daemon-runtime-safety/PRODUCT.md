...
1. For an authorized WeChat message, replaying the same stable message identity after its handling has settled does not repeat the user-visible or task side effects of `/status`, `/help`, `/new`, a natural-language new-conversation command, or an ordinary task. Ordinary tasks are durably accepted before the update cursor advances. Operational command replies enter the replay ledger only after their delivery settles, so a crash while a reply is in flight replays the idempotent command and re-delivers the reply instead of silently dropping it; duplicate copies inside one batch still produce a single reply. Long-running accepted tasks and slow native typing feedback continue in the background and do not block later polls.
...
4. The local daemon accepts only literal loopback listen addresses. Every HTTP route except the identifier-free `GET /healthz` probe requires the installation's private bearer credential; a missing or incorrect credential receives `401`, and responses and logs never disclose the credential. `/healthz` exposes only the service name and readiness booleans so installers can classify port ownership without a credential. Concurrent first use creates one shared private credential with owner-only permissions where the platform supports POSIX modes.
5. Listening on a socket is not readiness. The daemon starts non-ready, becomes ready only after a successful WeChat poll, becomes non-ready on polling or WeChat authentication errors, and recovers after a later successful poll. Authenticated business routes return `503` while non-ready. Installation checks readiness through the identifier-free `/healthz` probe and never puts the credential in process arguments.
