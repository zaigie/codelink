...
4. 已完成卸载后再次运行卸载仍须成功；幂等处理只接受带稳定标志的“服务、插件或 marketplace 已不存在”结果（按标志包含匹配，容忍版本措辞漂移），其他错误不能被忽略。Codex 可执行文件缺失不阻塞卸载：服务停止与 runtime 清理照常执行，插件与 marketplace 移除跳过并明确提示。
