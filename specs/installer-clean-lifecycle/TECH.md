...
3. 新增一个 Node 卸载编排模块，唯一负责固定顺序、Codex 命令执行、精确 absent 分类、错误上下文和 fail-closed。编排器同步串行执行，避免并发卸载产生顺序竞争；它不记录 token、session 或命令环境。
...
5. 服务不存在通过结构化查询或精确错误匹配识别。Codex `plugin remove` 的当前 absent 结果本身成功；marketplace 只忽略精确的 ``Error: marketplace `codelink-local` is not configured or installed``，其他非零退出均传播。
