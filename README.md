...
微信新建的不同会话使用各自独立的 thread ID，但统一以当前用户主目录下的 `Documents/Codex/CodeLink` 作为 Codex 项目目录；CodeLink 使用系统路径 API 生成它，在 Windows 上会得到对应的反斜杠路径，不再为每个会话生成时间戳目录。这样为 Codex 按 cwd 归组时提供稳定的 **CodeLink** 项目归属；远程界面的刷新时机和历史项目呈现仍由 Codex 管理。通过桌面通知绑定的已有会话仍保持它原来的项目目录。
